...
W&B Registry is a curated central repository of [artifact]({{< relref "/guides/artifacts/" >}}) versions within your organization. Users who [have permission]({{< relref "./configure_registry.md" >}}) within your organization can [download]({{< relref "./download_use_artifact.md" >}}), share, and collaboratively manage the lifecycle of all artifacts, regardless of the team that user belongs to.

You can use the Registry to [track artifact versions]({{< relref "./link_version.md" >}}), audit the history of an artifact's usage and changes, ensure governance and compliance of your artifacts, and [automate downstream processes such as model CI/CD]({{< relref "/guides/automations/" >}}).
...
- Track an [artifact’s lineage]({{< relref "/guides/registry/lineage.md" >}}) and audit the history of changes.
- [Automate]({{< relref "/guides/automations/" >}}) downstream processes such as model CI/CD.
...
To add an artifact to a registry, you first log a [specific artifact version to W&B]({{< relref "/guides/artifacts/create-a-new-artifact-version.md" >}}). Each time you log an artifact, W&B automatically assigns a version to that artifact. Artifact versions use 0 indexing, so the first version is `v0`, the second version is `v1`, and so on. 
